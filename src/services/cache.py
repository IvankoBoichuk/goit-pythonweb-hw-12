...
class CacheService:
    ...
    def get_contacts_cache(self, user_id: int) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if cached_data:
                ...
                logger.debug(
                    f"Found {len(contacts_data)} cached contacts for user {user_id}"
                )
    ...
    # === Методи для токенів скидання пароля ===

    def set_reset_token_cache(
        self, email: str, reset_token: str, ttl: timedelta = timedelta(hours=1)
    ) -> bool:
        """Кешує токен скидання пароля для захисту від brute force атак.

        Args:
            email (str): Email користувача
            reset_token (str): Токен скидання пароля
            ttl (timedelta): Час життя токену (за замовчуванням 1 година)
        ...
        try:
            # Зберігаємо токен з асоціацією до email
            key = self._generate_key("reset_token", reset_token)
            ttl_seconds = int(ttl.total_seconds())
            ...
            # Також кешуємо лічильник спроб скидання для email
            attempts_key = self._generate_key("reset_attempts", email)
            current_attempts = self.redis_client.get(attempts_key)
            new_attempts = (int(current_attempts) if current_attempts else 0) + 1

            # Максимум 3 спроби на годину
            self.redis_client.setex(attempts_key, 3600, str(new_attempts))

            logger.debug(f"Cached reset token for {email} (attempt #{new_attempts})")
            return True

        except Exception as e:
            logger.error(f"Failed to cache reset token for {email}: {e}")
            return False

    def get_reset_token_cache(self, reset_token: str) -> Optional[Dict[str, str]]:
        """Отримує дані токену скидання з кешу.

        Args:
            reset_token (str): Токен скидання пароля

        Returns:
            Optional[Dict]: Дані токену якщо знайдено, None в іншому випадку
        """
        if not self.cache_enabled or not self.redis_client:
            return None
        ...
        try:
            ...
            if cached_data:
                ...
                logger.debug(f"Found cached reset token for {token_data.get('email')}")
                return token_data

        except Exception as e:
            logger.error(f"Failed to get reset token {reset_token} from cache: {e}")

        return None

    def check_reset_attempts(self, email: str, max_attempts: int = 3) -> bool:
        """Перевіряє кількість спроб скидання пароля.

        Args:
            email (str): Email користувача
            max_attempts (int): Максимальна кількість спроб (за замовчуванням 3)

        Returns:
            bool: True якщо можна ще робити спроби, False якщо ліміт перевищено
        """
        if not self.cache_enabled or not self.redis_client:
            return True  # Якщо кеш недоступний, дозволяємо спробу
        ...
        try:
            ...
            if current_attempts:
                ...
                if attempts_count >= max_attempts:
                    logger.warning(
                        f"Reset attempts limit exceeded for {email}: {attempts_count}"
                    )
                    return False
            ...
            logger.error(f"Failed to check reset attempts for {email}: {e}")
            return True  # У випадку помилки дозволяємо спробу

    def invalidate_reset_token(self, reset_token: str, email: str = None) -> bool:
        """Інвалідує токен скидання пароля після використання.

        Args:
            reset_token (str): Токен скидання пароля
            email (str, optional): Email користувача для очищення лічильника
        ...
        try:
            keys_to_delete = [self._generate_key("reset_token", reset_token)]

            # Якщо передано email, також очищаємо лічильник спроб
            if email:
                keys_to_delete.append(self._generate_key("reset_attempts", email))

            deleted = self.redis_client.delete(*keys_to_delete)
            logger.debug(f"Invalidated reset token and cleared {deleted} cache entries")
            return True
